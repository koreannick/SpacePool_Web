...
if($uid){
	$result = mysql_query("select g1_d,g1_d_price,g1_s,g1_s_price,g3_t,g3_price,g4_person,g4_price,g4_hour1,g4_hour2,id,minHour,maxHour from es_product where uid=$uid");
	if($result&&mysql_num_rows($result)>0){
		...
		$g3_t = mysql_result($result,0,4);
		$g3_price = mysql_result($result,0,5);
		$g4_person = mysql_result($result,0,6);
		$g4_price = mysql_result($result,0,7);
		$g4_hour1 = mysql_result($result,0,8);
		$g4_hour2 = mysql_result($result,0,9);
		$id = mysql_result($result,0,10);
		$minHour = mysql_result($result,0,11);
		$maxHour = mysql_result($result,0,12);
			...
			<div class="con_wrap">
				<div class="con_in">
					<div class="s_view_wrap">
						<form name="form0" target="" method="post" action="space_apply3.php">
							...
							<div class="space_box">
								...
								<div class="space_box_input">
									<div class="input_table">
										<table width="100%" border="0" cellspacing="0" cellpadding="0" id="GroupOption">
											...
											<?
											if($uid){
												$result = mysql_query("select * from es_product_g2 where puid=$uid order by uid asc");
												if($result&&mysql_num_rows($result)>0){
													while($r=mysql_fetch_array($result)){
											?>
											<tr>
												<td align="left" valign="middle" class="tb_01">
													<select name="gName[]" id="gName_1">
														<option value="1" <?if($r["gName"]=="1")		echo " selected";?>>1인실</option>
														<option value="2" <?if($r["gName"]=="2")		echo " selected";?>>2인실</option>
														<option value="4" <?if($r["gName"]=="4")		echo " selected";?>>4인실</option>
														<option value="6" <?if($r["gName"]=="6")		echo " selected";?>>6인실이상</option>
														<option value="O" <?if($r["gName"]=="O")		echo " selected";?>>오픈스페이스</option>
														<option value="D" <?if($r["gName"]=="D")		echo " selected";?>>Desk</option>
													</select>
												</td>
												<td align="right" valign="middle" class="tb_02">
													<select name="gPeriod[]" id="gPeriod_1">
														<option value="M" <?if($r["gPeriod"]=="M")		echo " selected";?>>월</option>
														<option value="D" <?if($r["gPeriod"]=="D")		echo " selected";?>>일</option>
													</select>
												</td>
												<td align="right" valign="middle" class="tb_03">
													<input type="text" name="gPrice[]" id="gPrice_1" onkeypress="onlyNumber();" maxlength="10" style="ime-mode:disabled;" value="<?=$r["gPrice"]?>">
												</td>
											</tr>
											<?
													}
											...
											<tr>
												<td align="left" valign="middle" class="tb_01">
													<select name="gName[]" id="gName_1">
														...
														<option value="6">6인실이상</option>
														<option value="O">오픈스페이스</option>
												...
												<td align="right" valign="middle" class="tb_03">
													<input type="text" name="gPrice" id="gPrice_1" onkeypress="onlyNumber();" maxlength="10" style="ime-mode:disabled;">
												</td>
											</tr>
											<?
											}
								...
								<div class="space_box_input">
									<div class="sp_semi_wrap">
										<span>세미나/미팅룸 : 수용인원</span>
										<span>
											<input type="text" name="g4_person" id="g4_person" onkeypress="onlyNumber();" maxlength="4" style="ime-mode:disabled;" value="<?=$g4_person?>">명
