...
<tr>
	<td bgcolor="#E7E7E7"><table width="100%" cellpadding="0" cellspacing="1" border="0">
		<tr>
			...
			<td bgcolor="#FFFFFF" style="padding:5px;">&nbsp;<?if($faq_kind_arr){?>
					<select name="kind" class="inputbox_single">
						<?for($i=1;$i<=count($faq_kind_arr);$i++){
							if($kind==$i)			$sel = "selected";
							else						$sel = "";
						?>
						<option value="<?=$i?>" <?=$sel?>><?=$faq_kind_arr[$i]?></option>
						<?}?>
					</select>&nbsp;
				<?}?><input size=80 maxlength=70 name="subject" value="<?=$subject?>" class="inputbox"></td>
		</tr>
		<tr>
