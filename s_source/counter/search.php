...
//이전 접속 경로별로 그룹화 한다.
$result = mysql_query("SELECT search_word, count(*) as b FROM $table $where_query group by search_word order by b desc ");
$total_record = mysql_num_rows($result);
$i = 0;
while($row = mysql_fetch_array($result)){
	$search_word[$i] = $row["search_word"];
	$count[$i] = $row[1];
...
for($i = 0; $i < $total_record; $i++) {
	if($max_count)		$count_img_width = intval(($count[$i] / $max_count) * 140);
	else					$count_img_width = 0;
	...
	$count1 = number_format($count[$i]);
  	...
  	if($search_word[$i] == "")		$search_word1 = "검색어 없음";
	else									$search_word1 = $search_word[$i];
	...
	echo("<tr bgcolor=\"white\" height=\"23\" onMouseOver=\"this.style.backgroundColor='#F2F2F2';\"  onMouseOut=\"this.style.backgroundColor='#FFFFFF';\">
        <td style=\"padding:5px;\">$search_word1</td>
