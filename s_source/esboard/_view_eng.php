...
if($viewable==0){
	echo "<script>alert('No access.');location.href='/';</script>";
	exit;
}
$uid = (int)$uid;
$page = (int)$page;
...
if($row = mysql_fetch_array($result)){
	...
	$comment = preg_replace("/(\<img )([^\>]*)(\>)/i", "\\1 name='target_resize_image[]' onclick='image_window(this)' style='cursor:pointer;' \\2 \\3", $comment);
...
<div id="board-thumbnail-document">
    <div class="board-document-wrap">
		...
		if($board_type!="gallery"){
			if($file_list)			echo "<div class=\"board-attach\">Attachments : ".$file_list."</div>";
		}
		?>
    </div>
	<div class="board-comments-area">
        ...
        <div class="board-control">
            <div class="left"> <a href="#" onClick="go_list();return false;" class="board-thumbnail-button-small">List</a><?if($prev_link){?> <a href="<?=$prev_link?>" class="board-thumbnail-button-small">Prev</a><?}?><?if($next_link){?> <a href="<?=$next_link?>" class="board-thumbnail-button-small">Next</a><?}?> </div>
			<?
			//if($board_type=="bbs"){
				if($writable&&!$top){
			?>
            <div class="right"><?if($replable){?> <a href="#" onClick="reply();return false;" class="board-thumbnail-button-small">Reply</a><?}?> <a href="#" onClick="edit();return false;" class="board-thumbnail-button-small">Modify</a> <a href="#" onClick="del_check();return false;" class="board-thumbnail-button-small">Delete</a> </div>
			<?
				}
			//}
			?>
