...
if(!$sDate){
	...
	if($sDate<=date("Y-m-d")){
		if(!define("_BOOKING_")){
			redir_proc2("이용일자는 오늘이후로 선택해주세요.");
			exit;
		}else{
			redir_proc3("/".$uid,"이용일자는 오늘이후로 선택해주세요.");
			exit;
		}
	}
}
...
if($gubun==1){
	...
	if($result&&mysql_num_rows($result)>0){
		...
		redir_proc3("/".$uid,"존재하지 않는 예약형태입니다.");
		exit;
	}
	@mysql_free_result($result);
...
if($gubun==4){
	$total_price = $price*($eTime-$sTime)*$person+$e1_price+$e2_price;
}else{
	if($btype=="Space"){
		$total_price = $price+$e1_price+$e2_price;
	}else{
		$total_price = $price*$person+$e1_price+$e2_price;
	}
}
?>